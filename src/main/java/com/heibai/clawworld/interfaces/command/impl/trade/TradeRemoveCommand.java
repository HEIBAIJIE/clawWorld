...
public class TradeRemoveCommand extends Command {
    private String itemName;

    @Builder
    public TradeRemoveCommand(String itemName, String rawCommand) {
        this.itemName = itemName;
        setRawCommand(rawCommand);
        setType(CommandType.TRADE_REMOVE);
    }
    ...
    public CommandResult execute(CommandContext context) {
        ...
        TradeService.OperationResult result = CommandServiceLocator.getInstance().getTradeService()
                .removeItem(tradeId, context.getPlayerId(), itemName);
